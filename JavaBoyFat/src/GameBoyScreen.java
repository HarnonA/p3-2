...
class GameBoyScreen extends Frame implements ActionListener, ComponentListener, ItemListener {
	...
	public void setChannelEnable() {
		if ((applet.dmgcpu != null) && (applet.dmgcpu.soundChip.soundEnabled)) {
			applet.dmgcpu.soundChip.channel1Enable = soundChannel1Enable
					.getState();
			applet.dmgcpu.soundChip.channel2Enable = soundChannel2Enable
					.getState();
			applet.dmgcpu.soundChip.channel3Enable = soundChannel3Enable
					.getState();
			applet.dmgcpu.soundChip.channel4Enable = soundChannel4Enable
					.getState();
		}
	}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (command.equals("Open ROM")) {
			...
			if (fd.getFile() != null) {
				applet.cartridge = new Cartridge(fd.getDirectory()
						+ fd.getFile(), this);
				applet.dmgcpu = new Dmgcpu(applet.cartridge, applet.gameLink,
						this);
			...
			if (applet.dmgcpu != null) {
				...
				fd.show();
				applet.queueDebuggerCommand("c " + fd.getDirectory()
						+ fd.getFile());
				applet.dmgcpu.terminate = true;
			} else {
			...
			for (int r = 0; r < JavaBoy.schemeNames.length; r++) {
				if (JavaBoy.schemeNames[r] == command) {
					applet.dmgcpu.graphicsChip.backgroundPalette.setColours(
							JavaBoy.schemeColours[r][0],
							JavaBoy.schemeColours[r][1],
							JavaBoy.schemeColours[r][2],
							JavaBoy.schemeColours[r][3]);

					applet.dmgcpu.graphicsChip.obj1Palette.setColours(
							JavaBoy.schemeColours[r][4],
							JavaBoy.schemeColours[r][5],
							JavaBoy.schemeColours[r][6],
							JavaBoy.schemeColours[r][7]);

					applet.dmgcpu.graphicsChip.obj2Palette.setColours(
							JavaBoy.schemeColours[r][8],
							JavaBoy.schemeColours[r][9],
							JavaBoy.schemeColours[r][10],
	...
	public void setFrameState(int b){

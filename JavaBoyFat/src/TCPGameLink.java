...
/**
 * This class implements the serial communcation (Game Link) on the Gameboy
 * using a socket connection to a remote host. A server must bind to a port, and
 * a client must connect to the port, then the two emulated Gameboys will appear
 * to be connected via a serial link.
 */
...
class TCPGameLink extends GameLink implements Runnable {
	...
	public TCPGameLink(Frame parent) {
		...
		try {
			...
			new ModalDialog(parent, "Server startup succeeded!",
					"Game Link server running", "A client can now connect.");
			Thread t = new Thread(this);
			t.start();
			serverRunning = true;
		} catch (Exception e) {
			new ModalDialog(parent, "Server startup failed",
					"Cannot start server on port " + PORTNO,
	...
	public TCPGameLink(Frame parent, String ip) {
		...
		try {
			...
			new ModalDialog(parent, "Connect succeeded!",
			...
			new ModalDialog(parent, "Connect failed", "Cannot connect to host "
					+ ip + ":" + PORTNO,
		...
		while (((serverRunning) || (clientConnected)) && (!terminate)) {
			...
			try {
				...
				while ((data != -1) && (!terminate)) { /*
					...
					data = inStream.read();
					// System.out.println("Synched: " + clock + " : " +
					// dmgcpu.instrCount);

					// System.out.println(v + " " + dmgcpu.instrCount);
					if (dmgcpu != null) {
						while ( /*
								 * (v + 600 > dmgcpu.instrCount) || /*((clock !=
								 * -1) && (clock > dmgcpu.instrCount)) ||
								 */(!dmgcpu.interruptsEnabled)
								|| ((dmgcpu.ioHandler.registers[0x0F] & dmgcpu.INT_SER) != 0)
								&& (!terminate)) {
							try {
					...
					if (initial == 1) {
						...
						// System.out.println("--> " +
						// JavaBoy.unsign(dmgcpu.ioHandler.registers[0x01]));
			...
			new ModalDialog(parent, "Connection lost",
					"The connection with the other", "machine has been lost.");
		}

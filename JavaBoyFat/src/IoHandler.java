...
/**
 * This class handles all the memory mapped IO in the range FF00 - FF4F. It also
 * handles high memory accessed by the LDH instruction which is locted at FF50 -
 * FFFF.
 */
...
class IoHandler {
	...
	public short ioRead(int num) {
		...
		case 0x69: // GBC BG Sprite palette
			...
			if (dmgcpu.gbcFeatures) {
				...
				return dmgcpu.graphicsChip.gbcBackground[palNumber]
						.getGbcColours(
	...
	public void ioWrite(int num, short data) {
		...
		case 0x11: // Sound channel 1, length and wave duty
			if (soundOn) {
				dmgcpu.soundChip.channel1
						.setDutyCycle((JavaBoy.unsign(data) & 0xC0) >> 6);
				dmgcpu.soundChip.channel1
						.setLength(JavaBoy.unsign(data) & 0x3F);
			}
		...
		case 0x13: // Sound channel 1, frequency low
			...
			if (soundOn) {
				dmgcpu.soundChip.channel1.setFrequency(((int) (JavaBoy
						.unsign(registers[0x14]) & 0x07) << 8)
		...
		case 0x14: // Sound channel 1, frequency high
			...
			if (soundOn) {
				if ((registers[0x14] & 0x80) != 0) {
					dmgcpu.soundChip.channel1.setLength(JavaBoy
							.unsign(registers[0x11]) & 0x3F);
					dmgcpu.soundChip.channel1.setEnvelope(
		...
		case 0x18: // Sound channel 2, frequency low
			...
			if (soundOn) {
				dmgcpu.soundChip.channel2.setFrequency(((int) (JavaBoy
						.unsign(registers[0x19]) & 0x07) << 8)
		...
		case 0x19: // Sound channel 2, frequency high
			...
			if (soundOn) {
				if ((registers[0x19] & 0x80) != 0) {
					dmgcpu.soundChip.channel2.setLength(JavaBoy
							.unsign(registers[0x21]) & 0x3F);
					dmgcpu.soundChip.channel2.setEnvelope(
		...
		case 0x1A: // Sound channel 3, on/off
			if (soundOn) {
				if ((JavaBoy.unsign(data) & 0x80) != 0) {
					dmgcpu.soundChip.channel3.setVolume((JavaBoy
							.unsign(registers[0x1C]) & 0x60) >> 5);
				} else {
		...
		case 0x1D: // Sound channel 3, frequency lower 8-bit
			...
			if (soundOn)
				dmgcpu.soundChip.channel3.setFrequency(((int) (JavaBoy
						.unsign(registers[0x1E]) & 0x07) << 8)
		...
		case 0x1E: // Sound channel 3, frequency higher 3-bit
			...
			if (soundOn) {
				if ((registers[0x19] & 0x80) != 0) {
					dmgcpu.soundChip.channel3.setLength(JavaBoy
							.unsign(registers[0x1B]));
				}
				dmgcpu.soundChip.channel3.setFrequency(((int) (JavaBoy
						.unsign(registers[0x1E]) & 0x07) << 8)
						+ JavaBoy.unsign(registers[0x1D]));
		...
		case 0x23: // Sound channel 4, initial/consecutive
			...
			if (soundOn) {
				if ((registers[0x23] & 0x80) != 0) {
					dmgcpu.soundChip.channel4.setLength(JavaBoy
							.unsign(registers[0x20]) & 0x3F);
				}
		...
		case 0x3F:
			if (soundOn)
				dmgcpu.soundChip.channel3.setSamplePair(num - 0x30,
						JavaBoy.unsign(data));
		...
		case 0x46: // DMA
			...
			for (int i = 0x00; i < 0xA0; i++) {
				dmgcpu.addressWrite(0xFE00 + i,
						dmgcpu.addressRead(sourceAddress + i));
			}
		...
		case 0x55:
			if ((!hdmaRunning) && ((registers[0x55] & 0x80) == 0)
					&& ((data & 0x80) == 0)) {
				int dmaSrc = (JavaBoy.unsign(registers[0x51]) << 8)
						+ (JavaBoy.unsign(registers[0x52]) & 0xF0);
				int dmaDst = ((JavaBoy.unsign(registers[0x53]) & 0x1F) << 8)
				...
				for (int r = 0; r < dmaLen; r++) {
					dmgcpu.addressWrite(dmaDst + r,
							dmgcpu.addressRead(dmaSrc + r));
				}
			} else {

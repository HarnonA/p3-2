...
class Dmgcpu {
	...
	public void terminateProcess() {
		 ...
		 * System.out.println("Wating for CPU..."); } catch
		 * (InterruptedException e) { // Nothing } } while (running);
		 */
	}
	...
	public final void addressWrite(int addr, int data) {
		 ...
		 * (running)) { System.out.println(JavaBoy.hexWord(data) +
		...
		case 0xF000:
			if (addr < 0xFE00) {
				try {
					...
					System.out.println("Address error: " + addr + " pc = "
							+ JavaBoy.hexWord(pc));
				}
			} else if (addr < 0xFF00) {
	...
	public final void addressWriteOld(int addr, int data) {
		...
		if ((addr < 0x8000)) {
			...
			System.out.println("Attempt to write to address "
					+ JavaBoy.hexWord(addr));
		}
	}
	...
	public final void initiateInterrupts() {
		...
		if ((instrCount % INSTRS_PER_HBLANK) == 0) {
			...
			// LCY Coincidence
			// The +1 is due to the LCY register being just about to be
			// incremented
			...
			graphicsChip.notifyScanline(JavaBoy
					.unsign(ioHandler.registers[0x44]));
			ioHandler.registers[0x44] = (byte) (JavaBoy
					.unsign(ioHandler.registers[0x44]) + 1);
	...
	public final void execute(int numInstr) {
		...
		for (int r = 0; (r != numInstr) && (!terminate); r++) {
			...
			case 0x52: // Debug breakpoint (LD D, D)
				...
				// if the breakpoint is on in the debugger
				if (breakpointEnable) {
					terminate = true;
					System.out.println("- Breakpoint reached");
				} else {
			...
			case 0xE1: // POP HL
				pc++;
				hl = (JavaBoy.unsign(addressRead(sp + 1)) << 8)
						+ JavaBoy.unsign(addressRead(sp));
				sp += 2;
				break;
	...
	public String disassemble(int address, int numInstr) {
		...
		for (int r = 0; r < numInstr; r++) {
			...
			String instr = new String("Unknown Opcode! ("
					+ Integer.toHexString(JavaBoy.unsign(b1)) + ")");
			byte instrLength = 1;
			...
			case 0x1F:
				...
				break;
			case 0x20:
				instr = "JR NZ, " + JavaBoy.hexWord(address + 2 + offset)
						+ ": " + offset;
			...
			if ((JavaBoy.unsign(b1) >= 0x80) && (JavaBoy.unsign(b1) <= 0xBF)) {
				...
				instr = aluOperations[operation] + " A, "
						+ registerNames[sourceRegister];
			}
			...
			if (JavaBoy.unsign(b1) == 0xCB) {
				...
				case 0:
					...
					instr = shiftOperations[operation] + " "
							+ registerNames[sourceRegister];
					break;
				case 1:
			...
			System.out.print(JavaBoy.hexWord(address) + ": "
					+ JavaBoy.hexByte(JavaBoy.unsign(b1)));

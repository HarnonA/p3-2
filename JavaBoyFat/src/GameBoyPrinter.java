...
class GameBoyPrinter extends GameLink {
	...
	GameBoyPrinter() {
		...
		source = new MemoryImageSource(IMAGE_WIDTH, IMAGE_HEIGHT,
				new DirectColorModel(32, 0x00FF0000, 0x0000FF00, 0x000000FF,
						0xFF000000), imageData, 0, IMAGE_WIDTH);
	...
	void send(byte b) {
		System.out.print(b + " ");
		cpu.ioHandler.registers[0x01] = 0x00; // Acknowledge the byte by sending
												// a zero
	...
	void scrollImage() {
		...
		for (int y = 0; y < IMAGE_HEIGHT - 8; y++) {
			for (int x = 0; x < IMAGE_WIDTH; x++) {
				imageData[y * IMAGE_WIDTH + x] = imageData[(y + 8)
						* IMAGE_WIDTH + x];
			}
		}
	}
		...
		if ((getByte(0) == 136)) {
			...
			if ((bytesAvailable() == 16) && (dataSize > 0)) {
				for (int offs = 0; offs < 8; offs++) {
					...
					imageData[((tileY * 8 + offs) * IMAGE_WIDTH) + (tileX * 8)
							+ 0] = palette[((b1 & 0x80) >> 6)
							+ ((b2 & 0x80) >> 7)];
					imageData[((tileY * 8 + offs) * IMAGE_WIDTH) + (tileX * 8)
							+ 1] = palette[((b1 & 0x40) >> 5)

...
public class JavaBoy extends java.applet.Applet implements Runnable,
	...
	/**
	 * This array contains the actual data for the colour schemes. These are
	 * only using in DMG mode. The first four values control the BG palette, the
	...
	static public String hexWord(int w) {
		return new String(hexByte((w & 0x0000FF00) >> 8)
				+ hexByte(w & 0x000000FF));
	}
	...
	public void paint(Graphics g) {
		if (dmgcpu != null) {
			...
			if ((stripTimer > stripLength) && (!fullFrame)
				 ...
				 * (dmgcpu.graphicsChip.isFrameReady()) { g.setColor(new
				 * Color((int) (Math.random() * 255), (int) (Math.random() *
				 * 255), (int) (Math.random() * 255))); g.fillRect(0, 0,
				 * getSize().width, getSize().height); imageSizeChanged = false;
				 * if (fullFrame) { System.out.println("fullframe is " +
				 * fullFrame + " at "+ System.currentTimeMillis()); }
				 * 
				 * } }
				 */
				...
				if (dmgcpu.graphicsChip.isFrameReady()) {
					...
					bufferGraphics.fillRect(0, 0, getSize().width,
							getSize().height);
					...
					if (stripTimer >= stripLength - 10) {
						stripPos = getSize().height - 40
								+ ((stripTimer - (stripLength - 10)) * 4);
					}
					...
					if (stripTimer < stripLength) {
						if (stripTimer < stripLength / 2) {
							...
							bufferGraphics.drawString(
									"JavaBoy - Neil Millstone", 2,
									stripPos + 12);
							bufferGraphics.setColor(new Color(255, 255, 255));
							bufferGraphics.drawString(
									"www.millstone.demon.co.uk", 2,
									stripPos + 24);
							bufferGraphics.drawString("/download/javaboy", 2,
									stripPos + 36);
						} else {
							bufferGraphics.setColor(new Color(255, 255, 255));
							bufferGraphics.drawString(
									"ROM: " + cartridge.getCartName(), 2,
									stripPos + 12);
							bufferGraphics.drawString(
									"Double click for options", 2,
									stripPos + 24);
							bufferGraphics.drawString("Emulator version: "
									+ versionString, 2, stripPos + 36);
						}
					}
	...
	/**
	 * Execute a debugger command which can consist of many commands separated
	 * by semicolons
	 */
	 ...
	 * " (c) 2005 Neil Millstone (application)"); runningAsApplet = false;
	 * JavaBoy javaBoy = new JavaBoy("");
	 * 
	 * // javaBoy.mainWindow.addKeyListener(javaBoy); //
	 * javaBoy.mainWindow.addWindowListener(javaBoy); if (args.length > 0) { if
	 * (args[0].equals("server")) { javaBoy.gameLink = new TCPGameLink(null); }
	 * else if (args[0].equals("client")) { javaBoy.gameLink = new
	 * TCPGameLink(null, args[1]); } } //
	 * javaBoy.mainWindow.setGraphicsChip(javaBoy.dmgcpu.graphicsChip);
	 * 
	 * Thread p = new Thread(javaBoy); p.start(); }
	 */
	// ================================================================

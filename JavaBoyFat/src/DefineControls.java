...
public class DefineControls extends Frame implements KeyListener, WindowListener, ActionListener {
	...
	/*
	 * Sets window to adjust controls
	 */
	public DefineControls() {
		...
		for (int r = 0; r < 8; r++) {
			controlsField[r].setText(getKeyDesc(JavaBoy.keyCodes[r],
			(char) JavaBoy.keyCodes[r]) + " (" + JavaBoy.keyCodes[r] + ")");
		}
	...
	public void keyReleased(KeyEvent e) {
		...
		for (int r = 0; r < 8; r++) {
			if (e.getSource() == controlsField[r]) {
				controlsField[r].setText(getKeyDesc(e.getKeyCode(),
						e.getKeyChar())
						+ " (" + e.getKeyCode() + ")");

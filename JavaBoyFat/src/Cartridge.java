...
class Cartridge {	
	Sing singleton = Sing.getInstance();
	/**
	 * Translation between ROM size byte contained in the ROM header, and the
	 * number of 16Kb ROM banks the cartridge will contain
	 */
	 ...
	 * from the cartridge filename given. Loads via the web if JavaBoy is
	 * running as an applet
	 */
	
	//private static Cartridge cart = null;
	//private Cartridge(){
	//}
	
	//public static Cartridge getCart(String romFileNamee, Component component) {
	public Cartridge(String romFileName, Component component) {
		...
		//	if (!JavaBoy.runningAsApplet) {
			if(!singleton.getRunning()){
				System.out.println("AAAAXXXXXXXXXXXXXXXXXXXX");
				loadBatteryRam();
			}
	...
	public String getMapInfo() {
		...
		case 1 /* MBC1 */:
			return "MBC1: ROM bank " + JavaBoy.hexByte(currentBank)
					+ " mapped to " + " 4000 - 7FFFF";
		case 2 /* MBC1+RAM */:
		case 3 /* MBC1+RAM+BATTERY */:
			out = "MBC1: ROM bank " + JavaBoy.hexByte(currentBank)
					+ " mapped to " + " 4000 - 7FFFF.  ";
			if (mbc1LargeRamMode) {
		...
		case 5:
		case 6:
			return "MBC2: ROM bank " + JavaBoy.hexByte(currentBank)
					+ " mapped to 4000 - 7FFF";

		case 0x19:
		case 0x1C:
			return "MBC5: ROM bank " + JavaBoy.hexByte(currentBank)
					+ " mapped to 4000 - 7FFF";
	...
	public void restoreMapping() {
		if (savedBank != -1) {
			System.out.println("- ROM Mapping restored to bank "
					+ JavaBoy.hexByte(savedBank));
	 ...
	 * but actually write to the ROM. This is usefull for patching parts of
	 * code. Only used by the debugger.
	 */
	 ...
	 * supports it. The filename is the same as the ROM filename, but with a
	 * .SAV extention. # * Files are compatible with VGB-DOS.
	 */
	...
	public void saveBatteryRam() {
		...
		try {
			...
			System.out.println("Error saving battery RAM to '"
					+ saveRamFileName + "'");
		}
	}
	...
	public void outputCartInfo() {
		...
		String infoString = "ROM Info: Name = " + cartName + ", Size = "
				+ (numBanks * 128) + "Kbit, ";
	...
	public int lookUpCartSize(int sizeByte) {
		...
		while ((i < romSizeTable.length) && (romSizeTable[i][0] != sizeByte)) {
			System.out.println("XXXX"+ romSizeTable.length + " " + romSizeTable[i][0]);
			i++;
		}
...
class WebSaveRAM implements Runnable, DialogListener {
	...
	public WebSaveRAM(URL url, boolean save, Cartridge cart, Dmgcpu cpu,
			String username) {
		this.url = url;
		this.save = save;
		this.cart = cart;
	...
	public void run() {
		...
		try {
			...
			new ModalDialog(null, "Error!",
					"No save data can be found on the server!", e.toString());
		} catch (Exception e) {
			System.out.println("Error! " + e);
			new ModalDialog(null, "Error!",
	...
	public void saveRam() throws Exception {
		...
		String content = "romname=" + URLEncoder.encode(cart.getRomFilename())
				+ "&gamename=" + URLEncoder.encode(cart.getCartName())
				+ "&user=" + URLEncoder.encode(username) + "&datalength="
				+ (cart.getBatteryRamSize() * 2) + "&data0=" + saveData
				+ params;
	...
	public void loadRam() throws Exception {
		...
		String content = "gamename=" + URLEncoder.encode(cart.getCartName())
				+ "&romname=" + URLEncoder.encode(cart.getRomFilename());

 ...
 * of the graphics screen, including the background, window, and sprite layers.
...
class TileBasedGraphicsChip extends GraphicsChip {
		...
		for (int i = 0; i < 40; i++) {
			...
			if ((attributes & 0x80) >> 7 == priority) {
				...
				if (tiles[tileNum].invalid(spriteAttrib)) {
					tiles[tileNum].validate(videoRam, vidRamAddress,
							spriteAttrib);
				}

				if ((spriteAttrib & TILE_FLIPY) != 0) {
					if (doubledSprites) {
						tiles[tileNum].draw(back, spriteX, spriteY + 8,
								spriteAttrib);
					} else {
						tiles[tileNum].draw(back, spriteX, spriteY,
								spriteAttrib);
					}
				} else {
				...
				// System.out.println("Sprite " + i + ": " + spriteX + ", " +
				// spriteY);

				if (doubledSprites) {
					if (tiles[tileNum + 1].invalid(spriteAttrib)) {
						tiles[tileNum + 1].validate(videoRam,
								vidRamAddress + 16, spriteAttrib);
					}

					if ((spriteAttrib & TILE_FLIPY) != 0) {
						tiles[tileNum + 1].draw(back, spriteX, spriteY,
								spriteAttrib);
					} else {
						tiles[tileNum + 1].draw(back, spriteX, spriteY + 8,
								spriteAttrib);
					}
				}
			}
	...
	public void notifyScanline(int line) {
		...
		if (line == 0) {
			clearFrameBuffer();
			/* if (spritesEnabledThisFrame) */drawSprites(
					backBuffer.getGraphics(), 1);
			spritesEnabledThisFrame = spritesEnabled;
			windowStopLine = 144;
			windowEnableThisLine = winEnabled;
		}

		// SpritesEnabledThisFrame should be true if sprites were ever on this
		// frame
		if (spritesEnabled)
		...
		if (line == JavaBoy.unsign(dmgcpu.ioHandler.registers[0x4A]) + 1) { // Compare
																			// against
																			// WY
																			// reg
		...
		// Can't disable background on GBC (?!). Apperently not, according to
		// BGB
		...
		if (((yPixelOfs + line) % 8 == 4) || (line == 0)) {
			...
			for (int x = 0; x < 21; x++) {
				if (bgWindowDataSelect) {
					...
					attributeData = JavaBoy
							.unsign(videoRam[tileNumAddress + 0x2000]);
				} else {
					tileNumAddress = bgStartAddress
							+ (((y + yTileOfs) % 32) * 32)
							+ ((x + xTileOfs) % 32);
				...
				tiles[tileNum].draw(back, (8 * x) - xPixelOfs, (8 * y)
						- yPixelOfs, attribs);
			}
	...
	public boolean draw(Graphics g, int startX, int startY, Component a) {
		...
		// for (int r = 0; r < 384; r++) {
		// if (!spriteTiles[r].valid)
		// System.out.println("Generating image for tile " + r);
		 ...
		 * int yPixelOfs = JavaBoy.unsign(dmgcpu.ioHandler.registers[0x42]) % 8;
		 * 
		 * int bgStartAddress; if (hiBgTileMapAddress) { bgStartAddress =
		 * 0x1C00; /* 1C00 } else { bgStartAddress = 0x1800; }
		 * 
		 ...
		 * 
		 * 
		 * tileAddress = bgStartAddress + (((y + yTileOfs) % 32) * 32) + ((x +
		 * xTileOfs) % 32); attributeData = JavaBoy.unsign(videoRam[tileAddress
		 * + 0x2000]);
		 * 
		 * if (bgWindowDataSelect) { tileNum =
		 * JavaBoy.unsign(videoRam[tileAddress]); } else { tileNum = 256 +
		...
		if (winEnabled) {
			...
			for (int y = 0; y < 19 - (wy / 8); y++) {
				for (int x = 0; x < 21 - (wx / 8); x++) {
					...
					if (dmgcpu.gbcFeatures) {
						attribData = JavaBoy
								.unsign(videoRam[tileAddress + 0x2000]);
					...
					if (wy + y * 8 < windowStopLine) {
						if (tiles[tileNum].invalid(attribs)) {
							tiles[tileNum].validate(videoRam, tileDataAddress,
									attribs);
						}
						tiles[tileNum].draw(back, wx + x * 8, wy + y * 8,
								attribs);
					}
				}
			}
		...
		/*
		 * back.setColor(new Color(255, 255, 255)); back.fillRect(0, 0, 160,
		 * 144); for (int r = 0; r < 384; r++) { tiles[r].validate(videoRam, r
		 * << 4, TILE_BKG); tiles[r].draw(back, 8 * (r % 20), 8 * (r / 20),
		 * TILE_BKG); }
		 */
	...
	/**
	 * This class represents a tile in the tile data area. It contains images
	 * for a tile in each of it's three palettes and images that are flipped
	 * horizontally and vertically. The images are only created when needed, by
	 * calling updateImage(). They can then be drawn by calling draw().
	 */
	class GameboyTile {
		...
		/**
		 * True, if the tile's image in the image[] array is a valid
		 * representation of the tile as it appers in video memory.
		 */
		...
		public void allocateImage(int attribs, Component a) {
			source[attribs] = new MemoryImageSource(8 * magnify, 8 * magnify,
					new DirectColorModel(32, 0x00FF0000, 0x0000FF00,
							0x000000FF, 0xFF000000), imageData, 0, 8 * magnify);
		...
		/**
		 * Returns true if this tile does not contian a valid image for the tile
		...
		/**
		 * Create the image of a tile in the tile cache by reading the relevant
		...
		public void updateImage(byte[] videoRam, int offset, int attribs) {
			...
			if (offset == 0x31E0) {
				// System.out.println("window updated with " +
				// JavaBoy.hexByte(attribs) + " xflip = " + (attribs &
				// TILE_FLIPX) + "  yflip = " + (attribs & TILE_FLIPY));
			}
			...
			for (int y = 0; y < 8; y++) {
				for (int x = 0; x < 8; x++) {
					...
					/*
					 * switch (pixelColor) { case 0 : rgbValue = 0xFFFFFFFF;
					 * break; case 1 : rgbValue = 0xFFAAAAAA; break; case 2 :
					 * rgbValue = 0xFF555555; break; default : case 3 : rgbValue
					 * = 0xFF000000; break; }
					 */
					 ...
					 * || ( (attribs & TILE_OBJ2) != 0) ) ) { rgbValue &=
					 * 0x00FFFFFF; } else if ((entryNumber == 0) && ((attribs &
					 * (TILE_OBJ1 | TILE_OBJ2)) == 0)) { rgbValue &= 0x00FFFFFF;
					 * }
					 */

					for (int cy = 0; cy < magnify; cy++) {
						for (int cx = 0; cx < magnify; cx++) {
							imageData[(y * 8 * magnify * magnify)
									+ (cy * 8 * magnify) + (x * magnify) + cx] = rgbValue;
						}
					}
		...
		/**
		 * Draw the tile with the specified attributes into the graphics context
		 * given
		 */
		...
		/**
		 * Invalidate tile with the specified palette, including all flipped
		 * versions.
		 */
